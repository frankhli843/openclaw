...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const subagentGlobalInstructions = await loadSubagentInstructions(prepared.context.workspaceDir);
  if (subagentGlobalInstructions) {
    prepared.context.commandBody = prependSubagentInstructions(prepared.context.commandBody, subagentGlobalInstructions);
  }
