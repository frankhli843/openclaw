...
const deliverSubagentAnnouncement = vi.fn();
const loadSessionEntryByKey = vi.fn(() => ({ sessionId: "child-session-id" }));
const loadRequesterSessionEntry = vi.fn(() => ({ entry: { sessionId: "parent-session-id" } }));
...
vi.mock("./subagent-announce-output.js", () => ({
  ...
  applySubagentWaitOutcome: vi.fn(({ wait, outcome, startedAt, endedAt }: any) => ({
    outcome: wait?.status === "timeout" ? { status: "timeout" } : outcome ?? { status: "unknown" },
    startedAt,
    endedAt,
  })),
