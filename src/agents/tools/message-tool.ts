...
import { runWithDirectAction } from "../../infra/outbound/direct-action-context.frankclaw.js"; // frankclaw:
import {
...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      const isDirectAction = actionParams.directAction === true;
      const runAction = () =>
      ...
      try {
        result = isDirectAction ? await runWithDirectAction(runAction) : await runAction();
      } catch (error) {
