        ...
        try {
          ...
          const trackedPromise = task
            ...
            .catch((unexpectedError) => {
              // Final safety net: callbacks above should never throw, but if one does
              // (e.g. logger normalization regression), swallow it here so the gateway
              // process does not exit on a benign post-startup logging failure.
              try {
                log?.error?.(
                  `[${id}] channel monitor chain encountered unexpected error: ${formatErrorMessage(unexpectedError)}`,
                );
              } catch {
                // logger itself is broken; nothing more we can safely do
              }
            });
          handedOffTask = true;
