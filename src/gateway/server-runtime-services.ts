...
function recoverPendingOutboundDeliveries(params: {
  ...
  void runDeliveryRecovery().catch((err: unknown) =>
  ...
  setInterval(() => {
    void runDeliveryRecovery().catch((err: unknown) =>
      params.log.error(`Periodic delivery recovery failed: ${String(err)}`),
    );
...
function registerGateNotifyDiscordRuntime(params: {
  ...
  const gateChannel = params.cfg.agents?.defaults?.gateNotifyChannel;
  const gateOwner = params.cfg.agents?.defaults?.gateNotifyOwner;
  if (!gateChannel) {
    return;
  }
  void import("../channels/gate-notify-discord.js")
...
export function activateGatewayScheduledServices(params: {
  ...
  // frankclaw: gate-notify-discord registration
  registerGateNotifyDiscordRuntime({
    cfg: params.cfgAtStart,
    log: params.log,
  });
