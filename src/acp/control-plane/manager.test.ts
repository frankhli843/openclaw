...
describe("AcpSessionManager", () => {
  ...
  // Regression guard for the upstream acpx synthetic-end_turn bug. When acpx's
  // `runPromptTurn` timeout fallback (prompt-turn-*.js) synthesizes
  // `stopReason: "end_turn"` from any agent reply, a background task that only
  // streamed narration will end without any tool call. manager.core emits a
  // console.warn tagged `[acp-manager] suspected acpx synthetic end_turn` so
  // operational alerting can key off it.
  it("warns when a parented ACP background task ends with narration and no tool calls", async () => {
    await withAcpManagerTaskStateDir(async () => {
      const runtimeState = createRuntime();
      runtimeState.runTurn.mockImplementation(async function* () {
        yield {
          type: "text_delta" as const,
          stream: "output" as const,
          text: "I'll investigate the failing tests and push a fix.",
        };
        yield { type: "done" as const };
      });
      hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      ...
      hoisted.readAcpSessionEntryMock.mockImplementation((paramsUnknown: unknown) => {
        const sessionKey = (paramsUnknown as { sessionKey?: string }).sessionKey;
        if (sessionKey === "agent:codex:acp:child-narration-only") {
          return {
            sessionKey,
            storeSessionKey: sessionKey,
            entry: {
              sessionId: "child-narration-only",
              updatedAt: Date.now(),
              spawnedBy: "agent:quant:telegram:quant:direct:822430204",
              label: "Narration-only repro",
            },
            acp: readySessionMeta(),
          };
        }
        if (sessionKey === "agent:quant:telegram:quant:direct:822430204") {
          return {
            ...
            storeSessionKey: sessionKey,
            entry: {
              sessionId: "parent-narration-only",
              updatedAt: Date.now(),
            },
          };
        }
      ...
      try {
        ...
        await manager.runTurn({
          cfg: baseCfg,
          sessionKey: "agent:codex:acp:child-narration-only",
          text: "Push a fix for the failing CI job",
          mode: "prompt",
          requestId: "run-narration-only",
        });
        await flushMicrotasks();
        ...
        const warned = warnSpy.mock.calls.some((call) =>
          call.some(
            (arg) =>
              typeof arg === "string" &&
              arg.includes("suspected acpx synthetic end_turn") &&
              arg.includes("run-narration-only"),
          ),
        );
  ...
  it("does not warn about synthetic end_turn when the turn produced a tool call", async () => {
    await withAcpManagerTaskStateDir(async () => {
      const runtimeState = createRuntime();
      runtimeState.runTurn.mockImplementation(async function* () {
        yield {
      ...
      hoisted.readAcpSessionEntryMock.mockImplementation((paramsUnknown: unknown) => {
        const sessionKey = (paramsUnknown as { sessionKey?: string }).sessionKey;
        if (sessionKey === "agent:codex:acp:child-with-tools") {
          return {
            sessionKey,
            storeSessionKey: sessionKey,
            entry: {
              sessionId: "child-with-tools",
              updatedAt: Date.now(),
              spawnedBy: "agent:quant:telegram:quant:direct:822430204",
              label: "With tools",
            },
      ...
      try {
        ...
        await manager.runTurn({
          cfg: baseCfg,
          sessionKey: "agent:codex:acp:child-with-tools",
          text: "Run the fix",
          mode: "prompt",
          requestId: "run-with-tools",
        });
        await flushMicrotasks();

        const warned = warnSpy.mock.calls.some((call) =>
          call.some(
            (arg) => typeof arg === "string" && arg.includes("suspected acpx synthetic end_turn"),
          ),
        );
