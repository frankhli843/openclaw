...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
  ...
  // frankclaw: use Carbon devGuilds for instant slash command propagation.
  ...
  const upstreamCommandSpecsResult = await resolveDiscordProviderCommandSpecs({
    cfg,
    runtime,
    nativeEnabled,
    nativeSkillsEnabled,
  ...
  // frankclaw addition: append per-skill command specs from skills/commands/SKILL.md.
  // Done after upstream's resolveDiscordProviderCommandSpecs so existing overflow
  // trimming applies first; if appending pushes us over Discord's 100-command limit
  // we log a warning. Upstream's variable is destructured to commandSpecs and reassigned
  // (let-binding) so we can extend it without forking the upstream helper.
  let commandSpecs = upstreamCommandSpecsResult.commandSpecs;
  if (nativeEnabled) {
    ...
    if (commandSpecs.length > 100) {
      runtime.log?.(
        warn(
          `discord: ${commandSpecs.length} commands exceeds limit (100) after frankclaw skill append; some commands may fail to deploy.`,
        ),
      );
    }
  }
