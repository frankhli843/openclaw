...
// frankclaw: reactMessageDiscord used for 🛏️ DNR reaction below
...
export async function processDiscordMessage(
  ...
  if (!finalDispatchResult || !hasFinalInboundReplyDispatch(finalDispatchResult)) {
    // frankclaw: if no final reply was dispatched and we're in the Discord
    // DNR (quiet hours) window, react with 🛏️ so the sender knows the
    // message was received but deferred until next open window.
    try {
