...
// frankclaw: DNR enforcement helpers
import {
  dnrSuppressedJsonResult,
  isDiscordActionDnrSuppressed,
} from "./runtime.messaging.send-dnr.frankclaw.js";
...
export async function handleDiscordMessageSendAction(ctx: DiscordMessagingActionContext) {
  switch (ctx.action) {
    ...
    case "sendMessage": {
      ...
      const to = readStringParam(ctx.params, "to", { required: true });
      // frankclaw: short-circuit if Discord DNR (quiet hours) suppresses this send
      if (isDiscordActionDnrSuppressed(to, "sendMessage")) {
        return dnrSuppressedJsonResult("Suppressed by quiet hours. Will be delivered later.");
      }
    ...
    case "threadCreate": {
      ...
      if (isDiscordActionDnrSuppressed(`channel:${channelId}`, "threadCreate")) {
        return dnrSuppressedJsonResult("Suppressed by quiet hours.");
      }
